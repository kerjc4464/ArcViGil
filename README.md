...
### 2. 启动后端
进入 `ArcViGil` 文件夹下的 `ArcViGil-BackEnd/`：

- **Windows**：**双击 `start.bat`**。
- **macOS / Linux**：先去 [python.org](https://www.python.org/downloads/) 装 Python 3.13（3.10+ 即可），然后在终端里运行：
  ```
  cd ArcViGil-BackEnd
  chmod +x start.sh && ./start.sh
  ```
  脚本会自动建 `.venv` 虚拟环境并安装依赖。
...
5. 选择调度模式（**本地常驻** 24*7 惰性生成 / **Resend云端定时** 免常驻限30天，到点由Resend直发）
6. 点击「发送测试邮件」——收到邮件说明一切就绪
...
3. 后端存入 SQLite：
   - **本地常驻**：调度器每 10 秒检查到期任务，到点才调 LLM 生成
   - **Resend云端定时**：创建时立刻生成并提交 `scheduled_at` 到 Resend，到点由云端直发，本地 `created_at=trigger_at` 到点才注入
4. 时间到 → 后端加载 Soul 设定（含简称模糊匹配）+ 聊天记录 → 调用 LLM 生成信件 → 真实发送
...
| 离线调度 | 关闭 ST 后，已安排的信件仍会按时生成并发送（支持本地常驻/云端定时双模式） |
| 双调度模式 | **本地常驻** 惰性生成+动态同步 / **Resend云端定时** 创建时生成免常驻（限30天，超期/SMTP自动回退） |
| 多角色 Soul 设定 | souls/ 目录放角色设定文件（txt/md/json），发信时保持人设（支持简称模糊匹配+可用列表注入上下文） |
| 记忆闭环回流 | 已发信件自动注入上下文，AI 知道自己写过什么（云端消息到点才注入） |
| 任务雷达 | 面板实时查看待发任务，可暂停/删除/测试发信（云端显示 ☁️ 标识与Resend ID） |
| 邮件摘要 | 可选自动生成 70-100 字信件摘要，节省注入 Token |
| 动态上下文同步 | 下线前最新聊天内容可选同步给待执行任务（云端模式自动禁用） |
...
支持 txt / md / json / yaml 格式。角色输出发信标记时使用的名字需为已注册 Soul 的精确名（已自动注入可用列表到上下文）；后台也支持简称模糊匹配（如 `朱月` → `朱月·布伦史塔德.txt`）。
...
**Q: 邮件发不出去？**
- **Resend**：免费账号需确保发件地址为 `onboarding@resend.dev`，目标邮箱已在 Resend 后台验证；云端定时限30天，超期会自动回退本地
- **Resend SSL报错 `SSLEOFError`？** 为网络瞬断，插件已自动重试/回退，重试后多可成功
